...
function mergeObjectTypeCandidates(typeName, candidates, typeMergingOptions) {
    ...
    const astNode = astNodes.slice(1).reduce((acc, astNode) => (0, merge_1.mergeType)(astNode, acc, {
...
function mergeEnumTypeCandidates(typeName, candidates, typeMergingOptions) {
    ...
    const astNodes = pluck('astNode', candidates);
    const astNode = astNodes.slice(1).reduce((acc, astNode) => (0, merge_1.mergeEnum)(astNode, acc, {
...
function defaultEnumValueConfigMerger(candidates) {
    const preferred = candidates.find(({ type, transformedSubschema }) => (0, delegate_1.isSubschemaConfig)(transformedSubschema) && transformedSubschema.merge?.[type.name]?.canonical);
...
function defaultTypeCandidateMerger(candidates) {
    const canonical = candidates.filter(({ type, transformedSubschema }) => (0, delegate_1.isSubschemaConfig)(transformedSubschema)
